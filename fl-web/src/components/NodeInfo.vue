...
const is_center_node = ref(false); // 新增：用于标记是否为中心节点
...
const display_node_status = async (node: DbNode) => {
	...
	if (node === state.center) {
		is_center_node.value = true;
	} else {
		is_center_node.value = false;
	}// 判断是否为中心节点
	console.log("显示节点状态", is_center_node);
	if (node.is_connect) {
		try {
			const res = await axios.get("http://" + node.ip + ":" + node.port + "/status");
		...
		<div class="scrollable-content">
			...
			<div class="node-list">
				<div class="center-node">
					...
					<div class="node-item">
						<el-card style="width: 230px; height: 80px; margin: 10px; cursor: pointer"
				...
				<div class="db-node">
					...
					<div style="
							...
							margin-top: 15px;
						" v-loading="loading" element-loading-text="Loading..." element-loading-background="rgba(0, 0, 0, 0)">
						...
						<el-card style="width: 230px; height: 80px; margin: 10px; cursor: pointer"
							@click="display_node_status(state.user)">
							<span class="node-name">
						...
						<div v-for="(node, index) in state.db_node.filter(n => n.name !== state.user.name)" :key="index"
							class="node-item">
							<el-card style="
									width: 230px;
									height: 80px;
				...
				<div v-if="showOverlay" class="overlay">
					<div class="overlay-content">
						...
						<div class="node-status-container">
							...
							<!-- 中心节点才显示 MySQL 和 Nginx 状态 -->
							<div v-if="is_center_node && node_status.is_connect" class="status-row">
