...
class FlowerClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        self.logger.info(f"[第{current_round}轮初始] Loss: {float(data['loss']):.4f}, Acc: {float(data['acc']):.4f}")
        ...
        for epoch in range(5):  # 本地训练5个epoch
            ...
            self.logger.info(f"第{current_round}轮 - Epoch {epoch+1} - Loss: {epoch_loss:.4f}, Acc: {epoch_acc:.4f}")
        
        _,_,finally_data = self.evaluate(parameters,config)
        self.logger.info(f"[第{current_round}轮最终] Loss: {float(finally_data['loss']):.4f}, Acc: {float(finally_data['acc']):.4f}")
    ...
    def evaluate(self, parameters, config):
        ...
        with torch.no_grad():
            for inputs, labels in self.trainloader:
                ...
                total += labels.size(0)
                correct += (predicted == labels).sum().item()
        return float(total_loss / total),total,{"acc": correct / total,"loss": total_loss/total}
# 数据加载函数
