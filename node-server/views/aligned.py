...
@aligned.get("/")
def get_aligned(request: Request):
    session = request.query_params.get("session")
    return {"message": "aligned"}
...
def add_aligned(ruler: Ruler,request: Request):
    ...
    datacount = 0
    # 读取原始文件
    with open(f"data/original/{original_file}.csv", "r") as original_file_f, open(f"data/aligned/{file_name}.csv", "w") as aligned_file_f:
        ...
        for row in original_file:
            ...
            for r in ruler.operator:
                ...
                elif r.operator == "+" or r.operator == "-" or r.operator == "*" or r.operator == "/":
                    try:
                        data = []
