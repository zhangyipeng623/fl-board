...
def add(ruler_info: RulerInfo):
    ...
    for ruler in ruler_list:
        ...
        ruler_detail.append(RulerDetail(ruler_id=ruler_model.id,original_node=ruler.name_id,operator=json.dumps(op)))
    # try:
...
@ruler.get("/get")
def get(operator: Operator,request: Request):
    op = operator.operator
    field,userid,file_name = operator.field_value.split(",")
    user = User.select().where(User.id == userid).get()
    res = requests.get(f"http://{user.ip}:{user.port}/aligned/get_data?op={op}&field={field}&file_name={file_name}")
    if res.status_code == 200:
        return res.json()
    else:
...
def get_ruler_detail(request: Request):
    ...
    ruler_detail = RulerDetail.select().where(RulerDetail.ruler_id == ruler_id,RulerDetail.original_node == original_node).get()
    operator = json.loads(ruler_detail.operator)
    return json.dumps({"ruler_detail":{
