        ...
        try:
            ...
            if (
                ...
                .where(
                    ...
                    DataBase.username == user_info["username"],
                )
                .exists()
            ):
...
def get_list(request: Request):
    db_list = (
        ...
        .where(DataBase.user_id == request.query_params.get("node_id"))
...
def get_filed(request: Request):
    ...
    user_name = name_id[1]
    field = (
        DataBase.select(DataBase.field, DataBase.file_name, DataBase.user_id)
        .where(DataBase.db_name == db_name, DataBase.username == user_name)
        .get()
    )
    ...
    for f in fields:
        field_list.append(
            {"label": f, "value": f + "," + str(userid) + "," + file_name}
        )
