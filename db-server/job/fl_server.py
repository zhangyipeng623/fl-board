...
def create_custom_strategy(strategy_class: type, model: nn.Module,logger,job_id,device="cpu",**kwargs) -> type:
    """创建继承自指定策略类的自定义策略"""
...
def start(net_id,job_id,logger):
    ...
    strategy = create_custom_strategy(strategy_class, model,logger,job_id=job_id,device=device,
                fraction_fit=1.0,
